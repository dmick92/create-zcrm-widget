...
export const renderVersionWarning = (npmVersion: string) => {
  ...
  if (currentVersion.includes("beta")) {
    logger.warn("  You are using a beta version of create-zcrm-widget.");
    logger.warn("  Please report any bugs you encounter.");
  } else if (currentVersion !== npmVersion) {
    logger.warn("  You are using an outdated version of create-zcrm-widget.");
    logger.warn(
      "  Your version:",
      currentVersion + ".",
...
function checkForLatestVersion(): Promise<string> {
  return new Promise((resolve, reject) => {
    https
      .get(
        "https://registry.npmjs.org/-/package/create-zcrm-widget/dist-tags",
        (res) => {
...
export const getNpmVersion = () =>
  ...
  checkForLatestVersion().catch(() => {
    try {
      return execSync("npm view create-zcrm-widget version").toString().trim();
    } catch {
      return null;
    }
  });
